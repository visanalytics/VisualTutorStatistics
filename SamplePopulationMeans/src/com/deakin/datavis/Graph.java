...
public class Graph {
    ...
    public Graph(int drawX1, int drawY1, int drawX2, int drawY2, ArrayList<Integer> timesIA, double mean2, Color cFront, Color cBack, boolean show_curve, int min, int max, int distance) {
    ...
    public void paint(Graphics g, int rowNum, boolean showMean) {
        this.paint(g, rowNum);
        ...
        while (i <= columnNum) {
            ...
            String s = String.valueOf(this.min + i * this.distance);
            g.setColor(Color.WHITE);
            g.drawString(s, xb - g.getFontMetrics().stringWidth(s) / 2, (int)((double)this.y2 - (double)this.axisborder_size * 0.75 + (double)g.getFontMetrics().getHeight()));
            ++i;
        }
        g.setFont(new Font("Dialog", 0, 15));
        g2.setColor(Color.WHITE);
        int xb = this.x1 + this.axisborder_size + w * columnNum;
        String st = "inter-arrival";
        g.drawString(st, xb + 2, this.y2 - this.axisborder_size - g.getFontMetrics().getHeight() / 2);
        String st2 = "time (s)";
        g.drawString(st2, xb + 2, this.y2 - this.axisborder_size + g.getFontMetrics().getHeight() / 2);
        int i2 = rowNum;
        while (i2 >= 0) {
            int yl = this.y2 - this.axisborder_size - h * i2;
            if (i2 % 2 == 0) {
                String s = String.valueOf(i2);
                g.setColor(Color.WHITE);
                g.drawString(s, this.x1 + (int)((double)this.axisborder_size * 0.25) - (g.getFontMetrics().stringWidth(s) + 2), yl + g.getFontMetrics().getHeight() / 4);
            }
            --i2;
        }
        double ratio = (double)(this.x2 - this.x1 - this.axisborder_size + 1) / (double)(this.max - this.min);
        int lx = (int)((double)(this.x1 + this.axisborder_size + 1) + this.mean * ratio);
        g2.setStroke(new BasicStroke(2.5f));
        g.setFont(new Font("Dialog", 1, 24));
        g2.setColor(Color.WHITE);
        g2.drawLine(lx, this.y1 + g.getFontMetrics().getHeight() * 2, lx, this.y2 - this.axisborder_size);
        String str = "Mean = " + String.format("%.2f", this.mean);
        g.drawString(str, lx - g.getFontMetrics().stringWidth(str) / 2, this.y1 + g.getFontMetrics().getHeight());
    ...
    public void paint(Graphics g, int x1, int y1, int x2, int y2, Color cBack) {
        ...
        int lx = (int)((double)(x1 + this.axisborder_size + 1) + this.mean * ratio);
        g2.setColor(Color.LIGHT_GRAY);
        g2.drawLine(lx, (int)((double)y1 + (double)(y2 - y1) * 0.1), lx, y2 - this.axisborder_size);
    }
}
