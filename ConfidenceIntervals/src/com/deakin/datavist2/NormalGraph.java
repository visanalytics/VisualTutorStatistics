    ...
    public void paint(Graphics g, int rowNum) {
        ...
        if (this.show_curve) {
            ...
            while (i3 < itNum) {
                ...
                double yVal = this.normalDist.density((double)this.min + xVal - 0.5) * (double)this.distance * (double)this.size;
            ...
            while (z <= 3.0) {
                ...
                if (xVal <= (double)this.max && xVal >= (double)this.min) {
                    x = (double)(this.x1 + this.axisborder_size) + wx * (xVal + 0.5 - (double)this.min);
